...
**Always add unit tests when fixing bugs.** Extract logic into testable functions to verify the fix.

Example: When fixing a concurrency bug, extract the logic and test it:
...
test("prevents concurrent login attempts", async () => {
  ...
  expect(loginFn).toHaveBeenCalledTimes(1); // Only called once
});
```
